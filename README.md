...
**Frontend**: React + Vite · **Backend**: Node.js CLI
...
├── backend/           # Node.js CLI
│   ├── src/           # OrderManager, BotManager, logger, index
...
node src/index.js --simulate         # simulation → result.txt
```

CLI commands: `new_normal_order` · `new_vip_order` · `new_bot` · `del_bot` · `status` · `exit`

### Run scripts

```bash
bash script/build.sh   # install deps
bash script/test.sh    # 30 unit tests
bash script/run.sh     # simulate & write result.txt
```
