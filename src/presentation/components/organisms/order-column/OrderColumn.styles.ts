...
export const Header = styled.header<{ $tone: 'pending' | 'complete' }>`
  display: flex;
  align-items: center;
...
export const Accent = styled.span<{ $tone: 'pending' | 'complete' }>`
  width: 12px;
  height: 12px;
  border: 2px solid ${({ theme }) => theme.color.ink};
  ...
  background: ${({ theme, $tone }) =>
    $tone === 'pending'
      ? theme.color.pendingSolid
      : theme.color.completeSolid};
`;
