...
- Trivially unit-testable without React or waiting 10 real seconds (time is
  injected via `Scheduler` — see `.claude/patterns/core-domain.md`).
...
> Canonical pinned stack, commands, and guardrails live in `CLAUDE.md`;
> engineering standards in `AGENTS.md`; area recipes in `.claude/patterns/`. This
> PRD owns **product behaviour** — those files own the **how**. File naming is
