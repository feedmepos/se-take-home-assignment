...
function OrderHeader({ id }: { id: number }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xl font-black text-mcd-black leading-none">#{id}</span>
    </div>
  )
}
  ...
  if (order.status === 'PROCESSING') {
    return (
      <div className="bg-white rounded-2xl shadow-md border-l-4 border-mcd-red flex items-center gap-3 px-4 py-3">
        ...
        <span className="text-[10px] font-bold uppercase tracking-wide text-mcd-red shrink-0">Cooking</span>
      </div>
    )
  }
