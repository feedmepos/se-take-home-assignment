...
```
cmd/api/main.go                     entry point + composition root: urfave/cli v3 command tree, signal handling, wiring
...
Dependency rule: ports are declared by their consumer, not their implementer. `usecase` declares `OrderRepository`/`BotRepository`/`Clock`/`Logger`; `controller` separately declares its own `OrderUsecase`/`BotUsecase` ports (satisfied structurally by `*usecase.Usecase`) so the handler layer never imports usecase's internal port types. `internal/repository/memory` and `infrastructure/*` implement ports structurally with no upward imports. The composition root is `cmd/api/main.go`: it declares the `urfave/cli/v3` command tree (root `feedme` with a persistent `--processing-time`/`-t` flag sourced from `FEEDME_PROCESSING_TIME`, subcommands `interactive`/`demo`), constructs the concrete logger/clock/repository adapters and the `usecase.Usecase` in a `wire()` helper, wraps it in `controller.New(uc, uc)` (an API-style `Controller` with one method per action taking/returning `dto` types: `CreateOrder(dto.CreateOrderRequest)`, `AddBot`, `RemoveBot`, `GetStatus`, `Shutdown`), and each subcommand's Action dispatches to `controller.RunInteractive` / `controller.RunDemo`. `router.go` is a thin REPL router mapping input lines to Controller methods — the CLI analog of routes → handler endpoints. The handler layer (`internal/handler/controller`) is framework-free and imports no infrastructure or repository packages — the CLI framework and adapters appear only in `cmd/api/main.go`.
