...
1. **Real-time CI runtime:** `result.txt` is produced by the real CLI honouring real 10s cooks, so the scenario takes ~35 seconds in CI (authentic 10s gaps, matching the sample). Deliberate — do not swap in a fake clock to speed it up.
